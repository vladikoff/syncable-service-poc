...
var historyObserver = {
  ...
  onDeleteVisits: function (uri, visitTime, guid, reason) {
    L.log("onDeleteVisits", uri.asciiSpec, guid);
  },
...
let SyncableHistoryService = {
  ...
  mergeDataAndStartSyncing: function (syncType, listOfSyncData, syncChangeProcessor) {
    let deferred = defer();
    if (syncType !== SyncData.dataTypes.HISTORY) {
      deferred.resolve();
      return deferred.promise;
    }
    let self = this;

    return this.historyService.getAllItems().
    then(function (localHistoryItems) {
      ...
      var remoteHistoryItems = listOfSyncData.
        filter(function (syncData) { return syncData.syncType === SyncData.dataTypes.HISTORY; }).
      ...
      if (changes.localChanges.length > 0) self.processSyncChanges(syncType, changes.localChanges);
      // push up necessary remote changes
      if (changes.remoteChanges.length > 0) syncChangeProcessor.processSyncChanges(syncType, changes.remoteChanges);
      // register sync change processor to propagate additional changes
      self.changeProcessors[syncType] = syncChangeProcessor;
  ...
  stopSyncing: function (syncType) {
    ...
    delete this.changeProcessors[syncType];
  ...
  processSyncChanges: function (syncType, changes) {
    ...
    L.log("SyncableHistoryService.processSyncChanges", syncType, changes);
  }
};
