...
function createAddChange(itemInfo) {
  return SyncChange.create(
          ...
          SyncData.create(SyncData.dataTypes.HISTORY, itemInfo));
}
...
var historyObserver = {
  ...
  onDeleteVisits: function (uri, visitTime, guid, reason) {
    ...
    then(function (historyItem) {
      let change;
      if (historyItem.visits.length > 0) {
...
let SyncableHistoryService = {
  ...
  mergeDataAndStartSyncing: function (dataType, listOfSyncData, syncChangeProcessor) {
    ...
    then(function (localHistoryItems) {
      L.log("SyncableHistoryService.mergeDataAndStartSyncing #localItems", localHistoryItems.length, localHistoryItems);
      L.log("SyncableHistoryService.mergeDataAndStartSyncing #remoteItems", listOfSyncData.length, listOfSyncData);
      var remoteDeleteChanges = listOfSyncData.
        filter(function (syncData) { return syncData.isTombstone(); }).
        map(function (syncData) { return createDeleteChange(syncData); });
      self.applySyncChangesToLocalService(remoteDeleteChanges).
      then(function () {
  ...
  processSyncChanges: function (dataType, changes) {
    ...
    var remoteDeleteChanges = changes.
      filter(function (change) { return change.isDelete(); });
    return self.applySyncChangesToLocalService(remoteDeleteChanges).
    then(function () {
      let historyService = require('history-service').get();
      let remoteHistoryItems = changes.
          filter(function (change) { return !change.isDelete(); }).
          map(function (change) { return change.syncData.specifics; });
      return group(remoteHistoryItems.map(function (remoteItem) {
        // get the local history item corresponding to each each remote item
        return historyService.getHistoryInfoForUri(remoteItem.histUri)
      })).
      then(function (localHistoryItems) {
        localHistoryItems = localHistoryItems.filter(function (item) { return item !== null; });
        var changes = reconcileLocalItemsWithRemoteItems(localHistoryItems, remoteHistoryItems);
